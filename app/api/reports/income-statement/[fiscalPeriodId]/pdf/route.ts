...
import { generateIncomeStatementPdf } from "@/lib/utils/report-pdf/income-statement-pdf";
...
export async function GET(
  ...
  try {
    ...
    const reportData = await caller.reports.incomeStatement({
    ...
    // Generate PDF
    try {
      const pdfDoc = generateIncomeStatementPdf({
        workspace: {
          id: workspace.id,
          name: workspace.name,
      ...
      return new NextResponse(pdfBuffer, {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="Resultatrapport_${reportData.period.label.replace(/\s/g, "_")}.pdf"`,
        },
      });
    } catch (pdfError) {
      console.error("[Income Statement PDF] PDF generation failed", {
    ...
    console.error("[Income Statement PDF] Error", {
